...
- Limitation → Fix<!--이전 파트 한계 → 이번 파트 해결책--> 
    - prev: Standard deep RL can fail catastrophically when rewards are sparse/poorly shaped/temporally delayed. Because it often depend on stochastic method like epsilon-greedy which is ineffective for discovering long-horizon reward
...
- Flow of this part <!--(어떤 흐름으로 강의가 진행되는지, 강의의 흐름을 반영하는건 맞지만 어떻게 알고리즘을 발전해나가는지 정리)-->
    1. Probelm motivation
        - At the easy game like Pong or Breakout from Atari, it's easy to get a reward luckily
        - But the hard game like Montezuma's Revange, sparse + delayed + misleading incentives so agent can get stuck exploiting known reward loops
    ...
    3. Tractability
        ...
        1. Multi-armed bandits (Easy way): There's no state, just single time step and theoretically exist optimal
...
- Limitation → Fix<!--이전 파트 한계 → 이번 파트 해결책-->
    ...
    - fix: Use simple, theoretically principled bandit strategies that achieve optimal regret without soving the full POMDP
...
- Flow of this part <!--(어떤 흐름으로 강의가 진행되는지, 강의의 흐름을 반영하는건 맞지만 어떻게 알고리즘을 발전해나가는지 정리)-->
    - This part's goal: Minimize regret with strategies that are  probably close (Big-O) to Bayes-optimal POMDP solution, but without solving it
    1. UCB (Optimism)
        ...
        - Bonus is bigger for actions with higher uncertainty (usually approximated by low counts $N(a) which selecting that action$)
...
- Terminology Map <!--(용어 등치/정의)-->
    ...
    - Thompson sampling/Posterior sampling
        - UCB $\rightarrow$ 보너스로 uncertainty를 처리/Thompson $\rightarrow$ more Baeysian
        - 현재 posterior에서 전체 $\theta$를 샘플링후 샘플된 $\theta$가 진짜라고 가정하고 그 모델에서 최적 arm (기대보상의 최댓값)을 선택하고 실행. 관찰된 보상을 바탕으로 posterior 업데이트
    ...
    - Information gain style algorithm
        - equation: $\Delta(a) = \mathbb{E}\big[r(a^*) - r(a)\big]$
            ...
            - $a^*$는 진짜 최적 arm, 하지만 모르기 때문에 현재 posterior (belief) 아래에서 기대값으로 얼마나 손해일 것 같은지를 계산
...
- Terminology Map <!--(용어 등치/정의)-->
    - Bootstrapped DQN을 통해 posterior sampling을 흉내내는 방식
        - 진짜 posterior $p(Q|D)$를 정확히 가지고있기 어렵기 때문에 여러개의 Q를 ensemble로 만들어서 분포처럼 사용.
        - 각 head가 데이터/초기화 차이 때문에 약간씩 다른 Q를 학습함
        - 매 에피소드마다 head 하나를 뽑은 후 그 head로 행동하며 샘플된 Q로 행동을 근사함
## Part 6
- Limitation → Fix<!--이전 파트 한계 → 이번 파트 해결책-->
    - prev: Sparse reward에서 보상 최대화만 탐색하면 어떤 행동이 좋은지 구분할 신호가 없기때문에 긴 시퀀스가 필요한 보상에서는 운좋게 맞추기가 거의 불가능함
...
- Terminology Map <!--(용어 등치/정의)-->
    - Sparse reward: 보상이 대부분의 사간 및 상태에서 0이고, 특정 행동/상태에서만 가끔 1같은 보상이 나오는 환경
    - Info gain
        - $IG(z,y|a)$
