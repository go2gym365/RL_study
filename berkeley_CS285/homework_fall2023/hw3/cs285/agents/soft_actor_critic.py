...
class SoftActorCritic(nn.Module):
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            if self.use_entropy_bonus and self.backup_entropy:
                ...
                next_action_entropy = -log_p
                next_qs += self.temperature * next_action_entropy[None, :]

            # Compute the target Q-value
            target_values: torch.Tensor = reward[None, :] + self.discount * (1.0 - done.float())[None, :] * next_qs
    ...
    def entropy(self, action_distribution: torch.distributions.Distribution):
        ...
        # SAC의 목적 함수는 Q+ alpha*H 를 최대화하는것이기 때문에 엔트로피 자체가 정책 파라미터에 대해 미분가능해야함. 
        # 따라서 reparameterization trick을 사용해야하기 때문에 rsample을 사용
        # 엔트로피가 높으면 액션이 골고루 퍼져있다는 듰. 에이전트가 더 다양한 탐험 가능
    ...
    def actor_loss_reinforce(self, obs: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            action = action_distribution.sample((self.num_actor_samples,)) # 액션 자체는 미분 안함
            ...
            # obs를 액션 수만큼 맞춰서 복사
            obs_flat = obs.repeat(self.num_actor_samples, 1)
            # action 2차원으로 펼치기
            ...
            q_values = q_values.reshape(self.num_critic_networks, self.num_actor_samples, batch_size)
